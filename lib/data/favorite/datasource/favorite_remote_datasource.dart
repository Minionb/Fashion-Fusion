...
class FavoriteRemoteDataSourceImpl implements FavoriteRemoteDataSource {
  ...
  Future<FavoriteModel> get() async {
    ...
    if (response.statusCode == StatusCode.ok) {
      try {
        final FavoriteModel decodedJson =
            FavoriteModel.fromJson(json.decode(response.data));
        return decodedJson;
      } catch (e) {
  ...
  Future<Unit> delete(PutDeleteFavoriteModel model) async {
   final response = await apiConsumer
        .delete(EndPoints.deleteFavoriteItems, body: model.toJson());
  ...
  final response = await apiConsumer
        .put(EndPoints.putFavoriteItems, body: model.toJson());
    if (response.statusCode == 201 || response.statusCode == 200) {
      return Future.value(unit);
    } else {
      throw const ServerException();
